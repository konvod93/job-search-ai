...
> = {
  ...
  medical: [
    ...
    {
      value: "occupational_industrial_medicine",
      label:
        "Медицина промислових, будівельних та транспортних підприємств (лікар/фельдшер медпункту підприємства, лікар з медогляду водіїв — передрейсовий/післярейсовий, профпатолог, уповноважений з охорони праці медичного профілю — складання актів з травматизму, участь у розслідуванні виробничих травм, проведення медогляду та виявлення профзахворювань)",
    },
  ],
  education: [
  ...
  // Медпрацівника передрейсового огляду (лікар-нарколог / медсестра)
  // свідомо НЕ включено в passenger_dispatch_safety нижче — на відміну
  // від початкового припущення, реальна практика інша з двох причин:
  // 1) кандидат-медик шукає роботу за фахом у категорії "Медицина", а не
  //    в "Пасажирський транспорт" (хіба що цілеспрямовано хоче саме в
  //    депо/АТП — рідкісний випадок, не typical);
  // 2) комерційні АТП здебільшого НЕ тримають такого медика у власному
  //    штаті — передрейсовий огляд віддають на аутсорс зовнішньому
  //    медзакладу за договором. Отже вакансію публікує сам медзаклад, і
  //    його природне місце — категорія "Медицина", підкатегорія
  //    occupational_industrial_medicine (медицина промислових/
  //    транспортних підприємств — саме туди й піде такий фахівець, там
  //    його шукатимуть) — за потреби ще й crossListedCategories у
  //    "Пасажирський транспорт", якщо роботодавець хоче охопити й
  //    транспортну аудиторію.
  passenger_transport: [
    {
    ...
    },
    {
      ...
      label:
        "Управління, диспетчеризація та безпека (диспетчер пасажирського транспорту / навігатор, начальник маршруту / старший колони, контролер технічного стану автотранспорту)",
    },
  ],
  drivers: [
