    ...
    if (isLowTrust) {
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              "Ця роль передбачає прямий приватний найм (домашній персонал/водій/охорона тощо) — публікувати такі вакансії можуть лише верифіковані агенції (юридичні особи). Приватний найм фізособою чи ФОП тут не підтримується — для перевірки кандидатів (документи, мед- і психогляд) потрібна структура-посередник.",
          },
          { status: 403 },
        );
      }
