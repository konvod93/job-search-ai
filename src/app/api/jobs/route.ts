    ...
    if (isLowTrust) {
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              "Ця роль вимагає верифікованої юрособи — або через прямий приватний найм (домашній персонал/водій/охорона тощо, де потрібен посередник для перевірки кандидатів), або через регуляторний статус (каса/обмін валют/ломбард вимагають ліцензії, якої в ФОП не буває). Приватний найм фізособою чи ФОП тут не підтримується.",
          },
          { status: 403 },
        );
      }
